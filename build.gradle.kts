...
version = "${versionSplit[0]}${
    if (providers.gradleProperty("jetbrains.space.automation.versioning").get()
            .toBoolean() && System.getenv().containsKey("JB_SPACE_EXECUTION_NUMBER")
    ) ".${System.getenv("JB_SPACE_EXECUTION_NUMBER")}" else ""
    ...
    repositories {
        maven {
            ...
            url = uri(
                if (version.toString()
                        .endsWith("SNAPSHOT")
                ) providers.gradleProperty("jetbrains.space.packages.snapshots.url") else providers.gradleProperty("jetbrains.space.packages.releases.url")
            )
            // environment variables
            credentials {
                username = if (System.getenv()
                        .containsKey("JB_SPACE_CLIENT_ID")
                ) System.getenv("JB_SPACE_CLIENT_ID") else localProperties.getProperty("jetbrains.space.client.id")
                password = if (System.getenv()
                        .containsKey("JB_SPACE_CLIENT_SECRET")
                ) System.getenv("JB_SPACE_CLIENT_SECRET") else localProperties.getProperty("jetbrains.space.client.secret")
            }
        }
        ...
        maven {
            ...
            credentials {
                ...
                ) System.getenv("GITHUB_PASSWORD") else localProperties.getProperty("github.password")
            }
        }
    }
}
